...
const require_computable_context = require("./computable_context.cjs");
...
var JsExecutionContext = class JsExecutionContext {
	...
	*/
	constructor(scope, vm, deadlineSetter, featureFlags, computableEnv) {
		...
		if (computableEnv !== void 0) this.computableHelper = new require_computable_context.ComputableContextHelper(this, computableEnv.blockCtx, computableEnv.mlEnv, featureFlags, computableEnv.computableCtx);
		this.injectCtx();
	}
	resetComputableCtx() {
		(0, _milaboratories_ts_helpers.notEmpty)(this.computableHelper, "Computable context helper is not initialized").resetComputableCtx();
	}
	...
	runCallback(cbName, ...args) {
		...
		try {
			...
			return quickjs_emscripten.Scope.withScope((localScope) => {
	...
	injectCtx() {
		quickjs_emscripten.Scope.withScope((localScope) => {
			...
			this.vm.setProp(configCtx, "featureFlags", this.exportObjectUniversal(_platforma_sdk_model.JsRenderInternal.GlobalCfgRenderCtxFeatureFlags, localScope));
...
var ErrorRepository = class {
	...
	setAndRecreateForQuickJS(error) {
		const errorId = (0, node_crypto.randomUUID)();
	...
	getOriginal(quickJSError) {
		if (!(quickJSError instanceof quickjs_emscripten.errors.QuickJSUnwrapError)) {
			console.warn("ErrorRepo: quickJSError is not a QuickJSUnwrapError", (0, _milaboratories_pl_client.stringifyWithResourceId)(quickJSError));
			return quickJSError;
		}
		const cause = quickJSError.cause;
		if (!(typeof cause === "object" && cause !== null && "name" in cause && typeof cause.name === "string")) {
			console.warn("ErrorRepo: quickJSError.cause is not an Error (can be stack limit exceeded)", (0, _milaboratories_pl_client.stringifyWithResourceId)(quickJSError));
			return quickJSError;
		}
		...
		return new _milaboratories_pl_errors.PlQuickJSError(quickJSError, error);
	}
};
//#endregion
exports.JsExecutionContext = JsExecutionContext;
