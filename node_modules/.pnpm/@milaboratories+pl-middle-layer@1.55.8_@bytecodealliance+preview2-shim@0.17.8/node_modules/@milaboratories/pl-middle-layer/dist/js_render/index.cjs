...
function logOutputStatus(handle, renderedResult, stable, recalculationCounter, unstableMarker) {
	if (require_index.getDebugFlags().logOutputStatus && (require_index.getDebugFlags().logOutputStatus === "any" || !stable)) if (stable) console.log(`Stable output ${handle} calculated ${renderedResult !== void 0 ? "defined" : "undefined"}; (#${recalculationCounter})`);
...
function computableFromRF(env, ctx, fh, codeWithInfo, configKey, ops = {}) {
	...
	return _milaboratories_computable.Computable.makeRaw((cCtx) => {
		const { code, featureFlags } = codeWithInfo;
		if (require_index.getDebugFlags().logOutputRecalculations) console.log(`Block lambda recalculation : ${key} (${cCtx.changeSourceMarker}; ${cCtx.bodyInvocations} invocations)`);
		const scope = new quickjs_emscripten.Scope();
		...
		try {
			...
			const rCtx = new require_context.JsExecutionContext(scope, scope.manage(runtime.newContext()), (s) => {
				deadlineSettings = s;
			}, featureFlags, {
				computableCtx: cCtx,
				blockCtx: ctx,
			...
			if (Object.keys(toBeResolved).length === 0) {
				...
				if (require_index.getDebugFlags().logJsExecStat) console.log(`[jsExec] ${key}: ${JSON.stringify(rCtx.stats)}`);
			...
			if (require_index.getDebugFlags().logOutputStatus) console.log(`Output ${fh.handle} scaffold calculated (not all computables resolved yet).`);
			keepVmAlive = true;
			return {
				ir: toBeResolved,
				postprocessValue: (resolved, { unstableMarker, stable }) => {
					...
					if (require_index.getDebugFlags().logJsExecStat) console.log(`[jsExec] ${key} #${recalculationCounter}: ${JSON.stringify(rCtx.stats)}`);
...
function executeSingleLambda(quickJs, fh, codeWithInfo, ...args) {
	...
	try {
		...
		if (require_index.getDebugFlags().logJsExecStat) console.log(`[jsExec] ${fh.handle}: ${JSON.stringify(rCtx.stats)}`);
