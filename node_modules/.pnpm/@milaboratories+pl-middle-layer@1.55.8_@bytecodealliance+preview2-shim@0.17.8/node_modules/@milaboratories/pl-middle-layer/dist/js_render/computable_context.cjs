...
var ComputableContextHelper = class {
	...
	parsePObjectCollection(handle, errorOnUnknownField, prefix, ...resolveSteps) {
		...
		const accResult = require_p_object_collection.parseFinalPObjectCollection(acc, errorOnUnknownField, prefix, resolveSteps);
		const result = {};
		for (const [key, obj] of Object.entries(accResult)) result[key] = (0, _platforma_sdk_model.mapPObjectData)(obj, (d) => this.wrapAccessor(d));
		return result;
	}
	registerComputable(hPrefix, computable) {
		const fHandle = `${hPrefix}_${(0, node_crypto.randomUUID)()}`;
	...
	getBlobContentAsString(handle, range) {
		...
		return this.registerComputable("getBlobContentAsString", _milaboratories_computable.Computable.make((ctx) => this.env.driverKit.blobDriver.getDownloadedBlob(resourceInfo, ctx), { postprocessValue: async (value) => {
	...
	getBlobContentAsBase64(handle, range) {
		const resourceInfo = this.getAccessor(handle).resourceInfo;
		return this.registerComputable("getBlobContentAsBase64", _milaboratories_computable.Computable.make((ctx) => this.env.driverKit.blobDriver.getDownloadedBlob(resourceInfo, ctx), { postprocessValue: async (value) => {
			if (value === void 0) return void 0;
	...
	_resultPool = void 0;
	get resultPool() {
		if (this._resultPool === void 0) this._resultPool = (0, _milaboratories_ts_helpers.notEmpty)(this.blockCtx.getResultsPool, "getResultsPool")(this.requireComputableCtx);
	...
	getDataFromResultPool() {
		...
		return {
			...
			entries: collection.entries.map((e) => ({
				ref: e.ref,
				obj: (0, _platforma_sdk_model.mapPObjectData)(e.obj, (d) => this.wrapAccessor(d))
			}))
		};
	}
		...
		return {
			...
			entries: collection.entries.map((e) => ({
				...
				obj: {
					id: e.obj.id,
					spec: e.obj.spec,
					data: (0, _platforma_sdk_model.mapValueInVOE)(e.obj.data, (d) => this.wrapAccessor(d))
				}
			}))
		};
	}
	...
	getDataFromResultPoolByRef(blockId, exportName) {
		return (0, _platforma_sdk_model.mapPObjectData)(this.resultPool.getDataByRef(blockId, exportName), (acc) => this.wrapAccessor(acc));
	}
	createPFrame(def) {
		try {
			var _usingCtx$1 = require_usingCtx._usingCtx();
			const guard = _usingCtx$1.u(new _milaboratories_pl_model_common.PoolEntryGuard(this.env.driverKit.pFrameDriver.createPFrame(def.map((c) => (0, _platforma_sdk_model.mapPObjectData)(c, (d) => this.transformInputPData(d))))));
	...
	createPTable(def) {
		try {
			...
			const guard = _usingCtx3.u(new _milaboratories_pl_model_common.PoolEntryGuard(this.env.driverKit.pFrameDriver.createPTable((0, _platforma_sdk_model.mapPTableDef)(def, (c) => (0, _platforma_sdk_model.mapPObjectData)(c, (d) => this.transformInputPData(d))))));
			this.requireComputableCtx.addOnDestroy(guard.entry.unref);
			return guard.keep().key;
		} catch (_) {
			_usingCtx3.e = _;
	...
	createPTableV2(def) {
		try {
			...
			const guard = _usingCtx4.u(new _milaboratories_pl_model_common.PoolEntryGuard(this.env.driverKit.pFrameDriver.createPTableV2((0, _platforma_sdk_model.mapPTableDefV2)(def, (c) => (0, _platforma_sdk_model.mapPObjectData)(c, (d) => this.transformInputPData(d))))));
			this.requireComputableCtx.addOnDestroy(guard.entry.unref);
			return guard.keep().key;
		} catch (_) {
			_usingCtx4.e = _;
	...
	*/
	transformInputPData(d) {
		...
		else if ((0, _platforma_sdk_model.isDataInfo)(d)) return (0, _platforma_sdk_model.mapDataInfo)(d, (a) => this.getAccessor(a));
		else return d;
	}
	...
	wrapAccessor(accessor) {
		if (accessor === void 0) return void 0;
			...
			const nextHandle = (0, node_crypto.randomUUID)();
	...
	injectCtx(configCtx) {
		...
		const vm = parent.vm;
		quickjs_emscripten.Scope.withScope((localScope) => {
			...
			const isLegacyBlock = !(0, _platforma_sdk_model.checkBlockFlag)(this.featureFlags, "requiresModelAPIVersion", 2);
			...
			if ((0, _platforma_sdk_model.checkBlockFlag)(this.featureFlags, "supportsLazyState")) {
			...
			const requiredServiceNames = new Set((0, _milaboratories_pl_model_common.resolveRequiredServices)(this.featureFlags));
			const serviceFunctions = /* @__PURE__ */ new Map();
			const injectors = require_service_injectors.getServiceInjectors();
			for (const [key, serviceId] of Object.entries(_milaboratories_pl_model_common.Services)) {
				...
				try {
					...
					for (const [method, fn] of Object.entries(methods)) serviceFunctions.set((0, _milaboratories_pl_model_common.serviceFnKey)(serviceId, method), fn);
				} catch (e) {
					throw new _milaboratories_pl_model_common.ServiceInjectionError(`Failed to inject service "${serviceId}"`, { cause: e });
				}
			}
			...
			exportCtxFunction("getServiceMethods", (serviceIdHandle) => {
				const pfx = (0, _milaboratories_pl_model_common.serviceFnKey)(vm.getString(serviceIdHandle));
			...
			exportCtxFunction("callServiceMethod", function(serviceIdHandle, methodHandle, ...argHandles) {
				...
				const fn = serviceFunctions.get((0, _milaboratories_pl_model_common.serviceFnKey)(serviceId, method));
				if (!fn) throw new _milaboratories_pl_model_common.ServiceMethodNotFoundError(`Method "${method}" not found on service "${serviceId}"`);
...
//#endregion
exports.ComputableContextHelper = ComputableContextHelper;

//# sourceMappingURL=computable_context.cjs.map