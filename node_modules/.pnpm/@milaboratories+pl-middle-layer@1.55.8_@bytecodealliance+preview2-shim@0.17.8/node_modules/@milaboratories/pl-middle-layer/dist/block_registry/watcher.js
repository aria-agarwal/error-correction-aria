import { getDevV1PacketMtime, getDevV2PacketMtime } from "./registry.js";
...
var BlockUpdateWatcher = class extends PollComputablePool {
	http;
	...
	getKey(req) {
		switch (req.currentSpec.type) {
			...
			case "from-registry-v2": return `from_registry_v2_${canonicalize(req)}`;
		...
		try {
			...
			switch (cSpec.type) {
				...
				case "from-registry-v2": try {
					...
					if (channel === void 0) if (cSpec.channel === void 0) if (await registry.getLatestOverview(cSpec.id, StableChannel)) channel = StableChannel;
					else {
						if (await registry.getLatestOverview(cSpec.id, AnyChannel) === void 0) {
