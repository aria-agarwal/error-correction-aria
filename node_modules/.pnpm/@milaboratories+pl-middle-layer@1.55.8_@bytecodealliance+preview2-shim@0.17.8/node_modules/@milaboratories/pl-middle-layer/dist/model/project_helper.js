...
var ProjectHelper = class {
	...
	*/
	deriveArgsFromStorage(blockConfig, storageJson) {
		if (blockConfig.modelAPIVersion !== BLOCK_STORAGE_FACADE_VERSION) return { error: /* @__PURE__ */ new Error("deriveArgsFromStorage is only supported for model API version 2") };
		try {
			const result = executeSingleLambda(this.quickJs, blockConfig.blockLifecycleCallbacks[BlockStorageFacadeCallbacks.ArgsDerive], extractCodeWithInfo(blockConfig), storageJson);
	...
	*/
	derivePrerunArgsFromStorage(blockConfig, storageJson) {
		if (blockConfig.modelAPIVersion !== BLOCK_STORAGE_FACADE_VERSION) throw new Error("derivePrerunArgsFromStorage is only supported for model API version 2");
		try {
			const result = executeSingleLambda(this.quickJs, blockConfig.blockLifecycleCallbacks[BlockStorageFacadeCallbacks.PrerunArgsDerive], extractCodeWithInfo(blockConfig), storageJson);
	...
	calculateEnrichmentTargets(req) {
		...
		const args = req.args();
		return executeSingleLambda(this.quickJs, blockConfig.enrichmentTargets, extractCodeWithInfo(blockConfig), args);
	}
	...
	*/
	getInitialStorageInVM(blockConfig) {
		if (blockConfig.modelAPIVersion !== BLOCK_STORAGE_FACADE_VERSION) throw new Error("getInitialStorageInVM is only supported for model API version 2");
		try {
	...
	*/
	getStorageDebugViewInVM(blockConfig, rawStorageJson) {
		if (blockConfig.modelAPIVersion !== BLOCK_STORAGE_FACADE_VERSION) throw new Error("getStorageDebugViewInVM is only supported for model API version 2");
		try {
	...
	*/
	migrateStorageInVM(blockConfig, currentStorageJson) {
		if (blockConfig.modelAPIVersion !== BLOCK_STORAGE_FACADE_VERSION) return { error: "migrateStorageInVM is only supported for model API version 2" };
		try {
