...
async function duplicateProject(tx, sourceRid, options) {
	...
	tx.lock(newPrj);
	const ts = String(Date.now());
	const kvSkipPrefixes = [require_project_model.BlockArgsAuthorKeyPrefix];
	const kvSkipKeys = new Set([
		require_project_model.ProjectCreatedTimestamp,
		require_project_model.ProjectLastModifiedTimestamp,
		require_project_model.ProjectStructureAuthorKey
	]);
