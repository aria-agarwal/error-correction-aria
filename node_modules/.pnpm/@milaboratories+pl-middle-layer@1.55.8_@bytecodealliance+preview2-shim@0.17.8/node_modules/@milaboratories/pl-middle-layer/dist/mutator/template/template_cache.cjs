...
function hashLibV2(lib) {
	return (0, node_crypto.createHash)("sha256").update(_milaboratories_pl_model_backend.PlTemplateLibV1.type.name).update(_milaboratories_pl_model_backend.PlTemplateLibV1.type.version).update(lib.name).update(lib.version).update(lib.src).digest("hex");
}
function hashSoftwareV2(sw) {
	return (0, node_crypto.createHash)("sha256").update(_milaboratories_pl_model_backend.PlTemplateSoftwareV1.type.name).update(_milaboratories_pl_model_backend.PlTemplateSoftwareV1.type.version).update(sw.name).update(sw.version).update(sw.src).digest("hex");
}
function hashLibV3(lib) {
	return (0, node_crypto.createHash)("sha256").update(_milaboratories_pl_model_backend.PlTemplateLibV1.type.name).update(_milaboratories_pl_model_backend.PlTemplateLibV1.type.version).update(lib.name).update(lib.version).update(lib.sourceHash).digest("hex");
}
function hashSoftwareV3(sw) {
	return (0, node_crypto.createHash)("sha256").update(_milaboratories_pl_model_backend.PlTemplateSoftwareV1.type.name).update(_milaboratories_pl_model_backend.PlTemplateSoftwareV1.type.version).update(sw.name).update(sw.version).update(sw.sourceHash).digest("hex");
}
function flattenV2Tree(data) {
	...
	function processLib(lib) {
		...
		if (!seen.has(hash)) {
			...
			nodes.push({
				hash,
				create: (tx) => tx.createValue(_milaboratories_pl_model_backend.PlTemplateLibV1.type, JSON.stringify(_milaboratories_pl_model_backend.PlTemplateLibV1.fromV2Data(lib).data)),
				childHashes: []
			});
		}
		return hash;
	...
	function processSoftware(sw) {
		...
		if (!seen.has(hash)) {
			...
			nodes.push({
				hash,
				create: (tx) => {
					const swData = _milaboratories_pl_model_backend.PlTemplateSoftwareV1.fromV2Data(sw);
					const ref = tx.createStruct(_milaboratories_pl_model_backend.PlTemplateSoftwareV1.type, swData.data);
					tx.setKValue(ref, _milaboratories_pl_model_backend.PlTemplateSoftwareV1.metaNameKey, JSON.stringify(swData.name));
					tx.lock(ref);
					return ref;
				},
				childHashes: []
	...
	function processTemplate(tpl) {
		...
		for (const [libId, lib] of Object.entries(tpl.libs ?? {})) {
			...
			childHashes.push(h);
			children.push({
				fieldName: `${_milaboratories_pl_model_backend.PlTemplateV1.libPrefix}/${libId}`,
				hash: h
			});
		}
		for (const [swId, sw] of Object.entries(tpl.software ?? {})) {
			...
			childHashes.push(h);
			children.push({
				fieldName: `${_milaboratories_pl_model_backend.PlTemplateV1.softPrefix}/${swId}`,
				hash: h
			});
		}
		...
		for (const [tplId, sub] of Object.entries(tpl.templates ?? {})) {
			...
			childHashes.push(h);
			children.push({
				fieldName: `${_milaboratories_pl_model_backend.PlTemplateV1.tplPrefix}/${tplId}`,
				hash: h
			});
		}
		const h = (0, node_crypto.createHash)("sha256").update(_milaboratories_pl_model_backend.PlTemplateV1.type.name).update(_milaboratories_pl_model_backend.PlTemplateV1.type.version).update(tpl.hashOverride ?? "no-override").update(tpl.name).update(tpl.version).update(tpl.src);
		...
		nodes.push({
			hash,
			create: (tx, childRefs) => {
				const tplRef = tx.createStruct(_milaboratories_pl_model_backend.PlTemplateV1.type, JSON.stringify(_milaboratories_pl_model_backend.PlTemplateV1.fromV2Data(tpl).data));
				for (const child of children) {
					const fld = (0, _milaboratories_pl_client.field)(tplRef, child.fieldName);
					tx.createField(fld, "Input");
					tx.setField(fld, (0, _milaboratories_ts_helpers.notEmpty)(childRefs.get(child.hash), `missing child ref ${child.hash}`));
				}
				tx.lock(tplRef);
				if (!tpl.hashOverride) return tplRef;
				const overrideRef = tx.createStruct(_milaboratories_pl_model_backend.PlTemplateOverrideV1.type, JSON.stringify(_milaboratories_pl_model_backend.PlTemplateOverrideV1.fromV2Data(tpl)));
				const overrideFld = _milaboratories_pl_model_backend.PlTemplateOverrideV1.tplField(overrideRef);
...
function flattenV3Tree(data) {
	...
	function processLib(lib) {
		...
		if (!seen.has(hash)) {
			...
			nodes.push({
				hash,
				create: (tx) => tx.createValue(_milaboratories_pl_model_backend.PlTemplateLibV1.type, JSON.stringify(_milaboratories_pl_model_backend.PlTemplateLibV1.fromV3Data(lib, getSourceCode(lib.name, sources, lib.sourceHash)).data)),
				childHashes: []
			});
		}
		return hash;
	...
	function processSoftware(sw) {
		...
		if (!seen.has(hash)) {
			...
			nodes.push({
				hash,
				create: (tx) => {
					const swData = _milaboratories_pl_model_backend.PlTemplateSoftwareV1.fromV3Data(sw, getSourceCode(sw.name, sources, sw.sourceHash));
					const ref = tx.createStruct(_milaboratories_pl_model_backend.PlTemplateSoftwareV1.type, swData.data);
					tx.setKValue(ref, _milaboratories_pl_model_backend.PlTemplateSoftwareV1.metaNameKey, JSON.stringify(swData.name));
					tx.lock(ref);
					return ref;
	...
	function processTemplate(tpl) {
		...
		const h = (0, node_crypto.createHash)("sha256").update(_milaboratories_pl_model_backend.PlTemplateV1.type.name).update(_milaboratories_pl_model_backend.PlTemplateV1.type.version).update(tpl.hashOverride ?? "no-override").update(tpl.name).update(tpl.version).update(tpl.sourceHash);
		for (const child of children) h.update("child:" + child.fieldName + ":" + child.hash);
		const hash = h.digest("hex");
		if (seen.has(hash)) return hash;
		seen.add(hash);
		nodes.push({
			...
			create: (tx, childRefs) => {
				...
				const tplRef = tx.createStruct(_milaboratories_pl_model_backend.PlTemplateV1.type, JSON.stringify(_milaboratories_pl_model_backend.PlTemplateV1.fromV3Data(tpl, sourceCode).data));
				for (const child of children) {
					const fld = (0, _milaboratories_pl_client.field)(tplRef, child.fieldName);
					tx.createField(fld, "Input");
					tx.setField(fld, (0, _milaboratories_ts_helpers.notEmpty)(childRefs.get(child.hash), `missing child ref ${child.hash}`));
				...
				tx.lock(tplRef);
				if (!tpl.hashOverride) return tplRef;
				const overrideRef = tx.createStruct(_milaboratories_pl_model_backend.PlTemplateOverrideV1.type, JSON.stringify(_milaboratories_pl_model_backend.PlTemplateOverrideV1.fromV3Data(tpl)));
				const overrideFld = _milaboratories_pl_model_backend.PlTemplateOverrideV1.tplField(overrideRef);
				tx.createField(overrideFld, "Service");
				tx.setField(overrideFld, tplRef);
				tx.lock(overrideRef);
...
async function getOrCreateTemplateCache(pl) {
	...
	const existing = await pl.withReadTx("templateCache:check", async (tx) => {
		const fd = await tx.getFieldIfExists((0, _milaboratories_pl_client.field)(pl.clientRoot, TemplateCacheFieldName));
		return fd ? (0, _milaboratories_pl_client.ensureResourceIdNotNull)(fd.value) : void 0;
	});
	if (existing) {
	...
	const result = await pl.withWriteTx("templateCache:init", async (tx) => {
		const fd = await tx.getFieldIfExists((0, _milaboratories_pl_client.field)(pl.clientRoot, TemplateCacheFieldName));
		if (fd) return (0, _milaboratories_pl_client.ensureResourceIdNotNull)(fd.value);
		const cache = tx.createStruct(TemplateCacheType);
		tx.createField((0, _milaboratories_pl_client.field)(pl.clientRoot, TemplateCacheFieldName), "Dynamic", cache);
		tx.lock(cache);
		await tx.commit();
...
async function dropTemplateCache(pl) {
	await pl.withWriteTx("templateCache:drop", async (tx) => {
		const cacheField = (0, _milaboratories_pl_client.field)(pl.clientRoot, TemplateCacheFieldName);
...
async function runGc(pl, cacheRid, maxEntries = GC_MAX_ENTRIES) {
	return await pl.withWriteTx("templateCache:gc", async (tx) => {
		...
		for (let i = 0; i < toEvict; i++) {
			tx.removeField((0, _milaboratories_pl_client.field)(cacheRid, entries[i].hash));
...
function createBatchNodes(tx, cacheRid, batch, resolvedIds, newRefs, now) {
	for (const node of batch) {
		...
		tx.createField((0, _milaboratories_pl_client.field)(cacheRid, node.hash), "Dynamic", ref);
...
async function materialize(pl, cacheRid, rootHash, nodes, stat) {
	...
	const phase1 = await pl.withWriteTx("templateCache:materialize", async (tx) => {
		const [exists, countStr] = await Promise.all([Promise.all(allHashes.map((h) => tx.fieldExists((0, _milaboratories_pl_client.field)(cacheRid, h)))), tx.getKValueStringIfExists(cacheRid, ACCESS_COUNT_KEY)]);
		const newCount = (countStr ? parseInt(countStr, 10) : 0) + 1;
		const now = Date.now().toString();
		if (exists[allHashes.length - 1]) {
			const rootRid = (0, _milaboratories_pl_client.ensureResourceIdNotNull)((await tx.getField((0, _milaboratories_pl_client.field)(cacheRid, rootHash))).value);
		...
		if (hitIndices.length > 0) {
			const hitFields = await Promise.all(hitIndices.map((i) => tx.getField((0, _milaboratories_pl_client.field)(cacheRid, allHashes[i]))));
			for (let j = 0; j < hitIndices.length; j++) resolvedIds.set(allHashes[hitIndices[j]], (0, _milaboratories_pl_client.ensureResourceIdNotNull)(hitFields[j].value));
		}
		...
		tx.setKValue(cacheRid, ACCESS_COUNT_KEY, newCount.toString());
		await tx.commit();
		for (const [hash, ref] of newRefs) resolvedIds.set(hash, await (0, _milaboratories_pl_client.toGlobalResourceId)(ref));
		return {
			done: false,
...
async function loadTemplateCached(pl, spec, options) {
	...
	try {
		...
		switch (spec.type) {
			case "explicit":
				tplData = (0, _milaboratories_pl_model_backend.parseTemplate)(spec.content);
				break;
			case "prepared":
				tplData = spec.data;
				break;
		...
		if (require_index.getDebugFlags().logTemplateCacheStat) console.log(`[templateCache] ${JSON.stringify(stat)}`);
	}
}
/**
...
exports.GC_ACCESS_THRESHOLD = GC_ACCESS_THRESHOLD;
exports.GC_MAX_ENTRIES = GC_MAX_ENTRIES;
exports.TemplateCacheFieldName = TemplateCacheFieldName;
exports.TemplateCacheType = TemplateCacheType;
exports.cacheBlockPackTemplate = cacheBlockPackTemplate;
exports.dropTemplateCache = dropTemplateCache;
exports.flattenTemplateTree = flattenTemplateTree;
exports.getOrCreateTemplateCache = getOrCreateTemplateCache;
