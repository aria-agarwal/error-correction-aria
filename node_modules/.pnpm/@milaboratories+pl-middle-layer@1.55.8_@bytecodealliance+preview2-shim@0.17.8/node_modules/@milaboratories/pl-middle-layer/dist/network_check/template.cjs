...
async function runUploadTemplate(logger, pl, name) {
	const outputs = await runTemplate(pl, _platforma_sdk_workflow_tengo.Templates["check_network.upload_template"], true, (tx) => ({ name: tx.createValue(_milaboratories_pl_client.Pl.JsonObject, JSON.stringify(name)) }), ["greeting"]);
	try {
		return JSON.parse((0, _milaboratories_ts_helpers.notEmpty)((await getFieldValue(pl, outputs.greeting)).data?.toString()));
	} finally {
...
async function runUploadFile(logger, signer, lsDriver, uploadClient, pl, filePath) {
	...
	const result = await runTemplate(pl, _platforma_sdk_workflow_tengo.Templates["check_network.upload_blob"], true, (tx) => ({ file: tx.createValue(_milaboratories_pl_client.Pl.JsonObject, JSON.stringify(handle)) }), ["progress", "file"]);
	try {
		const progress = await getFieldValue(pl, result.progress);
		if ((0, _milaboratories_pl_drivers.isUpload)(progress)) {
			const uploadData = _milaboratories_pl_drivers.ImportFileHandleUploadData.parse(JSON.parse((0, _milaboratories_ts_helpers.notEmpty)(progress.data?.toString())));
			if ((0, _milaboratories_pl_drivers.isSignMatch)(signer, uploadData.localPath, uploadData.pathSignature)) await (0, _milaboratories_pl_drivers.uploadBlob)(logger, uploadClient, progress, uploadData, () => false, {
...
async function runDownloadFile(logger, pl, lsDriver, uploadClient, downloadClient, filePath) {
	const handle = await lsDriver.getLocalFileHandle(filePath);
	const outputs = await runTemplate(pl, _platforma_sdk_workflow_tengo.Templates["check_network.download_blob"], true, (tx) => ({ file: tx.createValue(_milaboratories_pl_client.Pl.JsonObject, JSON.stringify(handle)) }), ["progress", "file"]);
	try {
		const progress = await getFieldValue(pl, outputs.progress);
		await (0, _milaboratories_pl_drivers.uploadBlob)(logger, uploadClient, progress, _milaboratories_pl_drivers.ImportFileHandleUploadData.parse(JSON.parse((0, _milaboratories_ts_helpers.notEmpty)(progress.data?.toString()))), () => false, {
		...
		return await downloadClient.withBlobContent(fileInfo, {}, {}, async (content) => await (0, node_stream_consumers.text)(content));
	} finally {
...
async function runSoftware(pl) {
	const result = await runTemplate(pl, _platforma_sdk_workflow_tengo.Templates["check_network.run_hello_world"], true, (_) => ({}), ["greeting"]);
	try {
		return (0, _milaboratories_ts_helpers.notEmpty)((await getFieldValue(pl, result.greeting)).data?.toString());
	} finally {
		await deleteFields(pl, Object.values(result));
	}
}
...
async function runPythonSoftware(pl, name) {
	const result = await runTemplate(pl, _platforma_sdk_workflow_tengo.Templates["check_network.run_hello_world_py"], true, (tx) => ({ name: tx.createValue(_milaboratories_pl_client.Pl.JsonObject, JSON.stringify(name)) }), ["greeting"]);
	try {
		return (0, _milaboratories_ts_helpers.notEmpty)((await getFieldValue(pl, result.greeting)).data?.toString());
	} finally {
		await deleteFields(pl, Object.values(result));
...
async function createBigTempFile() {
	const filePath = node_path.default.join(node_os.default.tmpdir(), `check-network-big-temp-${Date.now()}.bin`);
	const fileContent = (0, node_crypto.randomBytes)(20 * 1024 * 1024);
	await node_fs_promises.default.appendFile(filePath, fileContent);
	return { filePath };
}
/** Creates a temporarly file we could use for uploading and downloading. */
async function createTempFile() {
	const filePath = node_path.default.join(node_os.default.tmpdir(), `check-network-temp-${Date.now()}.txt`);
	const fileContent = "Hello, world! " + (/* @__PURE__ */ new Date()).toISOString();
	await node_fs_promises.default.writeFile(filePath, fileContent);
	return {
		filePath,
		fileContent
	};
...
async function runTemplate(client, tpl, ephemeral, inputs, outputs) {
	return await client.withWriteTx("TemplateRender", async (tx) => {
		const outputFields = require_render_template.createRenderTemplate(tx, require_template_loading.loadTemplate(tx, await require_template_loading.prepareTemplateSpec(tpl)), ephemeral, inputs(tx), outputs);
		const outputsIds = {};
		for (const output of outputs) {
			const fieldRef = (0, _milaboratories_pl_client.field)(client.clientRoot, output);
			tx.createField(fieldRef, "Dynamic", outputFields[output]);
			outputsIds[output] = await (0, _milaboratories_pl_client.toGlobalFieldId)(fieldRef);
		}
		await tx.commit();
		return outputsIds;
	});
...
async function getFieldValue(client, fieldId) {
	return await (0, _milaboratories_pl_client.poll)(client, async (tx) => {
		...
		if ((0, _milaboratories_pl_client.isNotNullResourceId)(field.error)) {
		...
		if ((0, _milaboratories_pl_client.isNullResourceId)(field.value)) throw new _milaboratories_pl_client.ContinuePolling();
...
exports.downloadFromEveryStorage = downloadFromEveryStorage;
exports.pythonSoftware = pythonSoftware;
