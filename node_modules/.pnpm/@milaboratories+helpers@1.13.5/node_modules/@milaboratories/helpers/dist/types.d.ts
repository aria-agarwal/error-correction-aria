...
export type OkType<R extends Result<unknown>> = Extract<R, {
    ok: true;
}>["value"];
...
export type DeepMutable<T> = keyof T extends never ? T : {
...
export type Objectify<U extends {
    key: string;
    value: unknown;
}> = {
...
export type UnionToTuples<T, A extends T[] = []> = UnionToTuple<T>["length"] extends A["length"] ? [...A] : UnionToTuples<T, [T, ...A]>;
export {};
