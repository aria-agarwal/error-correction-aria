import { ServiceType } from "@protobuf-ts/runtime-rpc";
import { MessageType, UnknownFieldHandler, WireType, reflectionMergePartial } from "@protobuf-ts/runtime";
//#region src/proto-grpc/github.com/milaboratory/pl/controllers/shared/grpc/uploadapi/protocol.ts
/**
* @generated from protobuf enum MiLaboratories.Controller.Shared.UploadAPI.ChecksumAlgorithm
*/
let UploadAPI_ChecksumAlgorithm = /* @__PURE__ */ function(UploadAPI_ChecksumAlgorithm) {
	/**
	* @generated from protobuf enum value: CHECKSUM_ALGORITHM_UNSPECIFIED = 0;
	*/
	UploadAPI_ChecksumAlgorithm[UploadAPI_ChecksumAlgorithm["UNSPECIFIED"] = 0] = "UNSPECIFIED";
	/**
	* @generated from protobuf enum value: CHECKSUM_ALGORITHM_CRC32C = 1;
	*/
	UploadAPI_ChecksumAlgorithm[UploadAPI_ChecksumAlgorithm["CRC32C"] = 1] = "CRC32C";
	return UploadAPI_ChecksumAlgorithm;
}({});
var UploadAPI$Type = class extends MessageType {
	constructor() {
		super("MiLaboratories.Controller.Shared.UploadAPI", []);
	}
	create(value) {
		const message = globalThis.Object.create(this.messagePrototype);
		if (value !== void 0) reflectionMergePartial(this, message, value);
		return message;
	}
	internalBinaryRead(reader, length, options, target) {
		let message = target ?? this.create(), end = reader.pos + length;
		while (reader.pos < end) {
			let [fieldNo, wireType] = reader.tag();
			switch (fieldNo) {
				default:
					let u = options.readUnknownField;
					if (u === "throw") throw new globalThis.Error(`Unknown field ${fieldNo} (wire type ${wireType}) for ${this.typeName}`);
					let d = reader.skip(wireType);
					if (u !== false) (u === true ? UnknownFieldHandler.onRead : u)(this.typeName, message, fieldNo, wireType, d);
			}
		}
		return message;
	}
	internalBinaryWrite(message, writer, options) {
		let u = options.writeUnknownFields;
		if (u !== false) (u == true ? UnknownFieldHandler.onWrite : u)(this.typeName, message, writer);
		return writer;
	}
};
new UploadAPI$Type();
var UploadAPI_Init$Type = class extends MessageType {
	constructor() {
		super("MiLaboratories.Controller.Shared.UploadAPI.Init", []);
	}
	create(value) {
		const message = globalThis.Object.create(this.messagePrototype);
		if (value !== void 0) reflectionMergePartial(this, message, value);
		return message;
	}
	internalBinaryRead(reader, length, options, target) {
		let message = target ?? this.create(), end = reader.pos + length;
		while (reader.pos < end) {
			let [fieldNo, wireType] = reader.tag();
			switch (fieldNo) {
				default:
					let u = options.readUnknownField;
					if (u === "throw") throw new globalThis.Error(`Unknown field ${fieldNo} (wire type ${wireType}) for ${this.typeName}`);
					let d = reader.skip(wireType);
					if (u !== false) (u === true ? UnknownFieldHandler.onRead : u)(this.typeName, message, fieldNo, wireType, d);
			}
		}
		return message;
	}
	internalBinaryWrite(message, writer, options) {
		let u = options.writeUnknownFields;
		if (u !== false) (u == true ? UnknownFieldHandler.onWrite : u)(this.typeName, message, writer);
		return writer;
	}
};
new UploadAPI_Init$Type();
var UploadAPI_Init_Request$Type = class extends MessageType {
	constructor() {
		super("MiLaboratories.Controller.Shared.UploadAPI.Init.Request", [{
			no: 1,
			name: "resource_id",
			kind: "scalar",
			T: 4,
			L: 0
		}, {
			no: 2,
			name: "resource_signature",
			kind: "scalar",
			opt: true,
			T: 12
		}]);
	}
	create(value) {
		const message = globalThis.Object.create(this.messagePrototype);
		message.resourceId = 0n;
		if (value !== void 0) reflectionMergePartial(this, message, value);
		return message;
	}
	internalBinaryRead(reader, length, options, target) {
		let message = target ?? this.create(), end = reader.pos + length;
		while (reader.pos < end) {
			let [fieldNo, wireType] = reader.tag();
			switch (fieldNo) {
				case 1:
					message.resourceId = reader.uint64().toBigInt();
					break;
				case 2:
					message.resourceSignature = reader.bytes();
					break;
				default:
					let u = options.readUnknownField;
					if (u === "throw") throw new globalThis.Error(`Unknown field ${fieldNo} (wire type ${wireType}) for ${this.typeName}`);
					let d = reader.skip(wireType);
					if (u !== false) (u === true ? UnknownFieldHandler.onRead : u)(this.typeName, message, fieldNo, wireType, d);
			}
		}
		return message;
	}
	internalBinaryWrite(message, writer, options) {
		if (message.resourceId !== 0n) writer.tag(1, WireType.Varint).uint64(message.resourceId);
		if (message.resourceSignature !== void 0) writer.tag(2, WireType.LengthDelimited).bytes(message.resourceSignature);
		let u = options.writeUnknownFields;
		if (u !== false) (u == true ? UnknownFieldHandler.onWrite : u)(this.typeName, message, writer);
		return writer;
	}
};
/**
* @generated MessageType for protobuf message MiLaboratories.Controller.Shared.UploadAPI.Init.Request
*/
const UploadAPI_Init_Request = new UploadAPI_Init_Request$Type();
var UploadAPI_Init_Response$Type = class extends MessageType {
	constructor() {
		super("MiLaboratories.Controller.Shared.UploadAPI.Init.Response", [
			{
				no: 1,
				name: "parts_count",
				kind: "scalar",
				T: 4,
				L: 0
			},
			{
				no: 3,
				name: "part_size",
				kind: "scalar",
				T: 4,
				L: 0
			},
			{
				no: 4,
				name: "checksum_algorithm",
				kind: "enum",
				T: () => [
					"MiLaboratories.Controller.Shared.UploadAPI.ChecksumAlgorithm",
					UploadAPI_ChecksumAlgorithm,
					"CHECKSUM_ALGORITHM_"
				]
			},
			{
				no: 5,
				name: "checksum_header",
				kind: "scalar",
				T: 9
			},
			{
				no: 2,
				name: "uploaded_parts",
				kind: "scalar",
				repeat: 1,
				T: 4,
				L: 0
			}
		]);
	}
	create(value) {
		const message = globalThis.Object.create(this.messagePrototype);
		message.partsCount = 0n;
		message.partSize = 0n;
		message.checksumAlgorithm = 0;
		message.checksumHeader = "";
		message.uploadedParts = [];
		if (value !== void 0) reflectionMergePartial(this, message, value);
		return message;
	}
	internalBinaryRead(reader, length, options, target) {
		let message = target ?? this.create(), end = reader.pos + length;
		while (reader.pos < end) {
			let [fieldNo, wireType] = reader.tag();
			switch (fieldNo) {
				case 1:
					message.partsCount = reader.uint64().toBigInt();
					break;
				case 3:
					message.partSize = reader.uint64().toBigInt();
					break;
				case 4:
					message.checksumAlgorithm = reader.int32();
					break;
				case 5:
					message.checksumHeader = reader.string();
					break;
				case 2:
					if (wireType === WireType.LengthDelimited) for (let e = reader.int32() + reader.pos; reader.pos < e;) message.uploadedParts.push(reader.uint64().toBigInt());
					else message.uploadedParts.push(reader.uint64().toBigInt());
					break;
				default:
					let u = options.readUnknownField;
					if (u === "throw") throw new globalThis.Error(`Unknown field ${fieldNo} (wire type ${wireType}) for ${this.typeName}`);
					let d = reader.skip(wireType);
					if (u !== false) (u === true ? UnknownFieldHandler.onRead : u)(this.typeName, message, fieldNo, wireType, d);
			}
		}
		return message;
	}
	internalBinaryWrite(message, writer, options) {
		if (message.partsCount !== 0n) writer.tag(1, WireType.Varint).uint64(message.partsCount);
		if (message.uploadedParts.length) {
			writer.tag(2, WireType.LengthDelimited).fork();
			for (let i = 0; i < message.uploadedParts.length; i++) writer.uint64(message.uploadedParts[i]);
			writer.join();
		}
		if (message.partSize !== 0n) writer.tag(3, WireType.Varint).uint64(message.partSize);
		if (message.checksumAlgorithm !== 0) writer.tag(4, WireType.Varint).int32(message.checksumAlgorithm);
		if (message.checksumHeader !== "") writer.tag(5, WireType.LengthDelimited).string(message.checksumHeader);
		let u = options.writeUnknownFields;
		if (u !== false) (u == true ? UnknownFieldHandler.onWrite : u)(this.typeName, message, writer);
		return writer;
	}
};
/**
* @generated MessageType for protobuf message MiLaboratories.Controller.Shared.UploadAPI.Init.Response
*/
const UploadAPI_Init_Response = new UploadAPI_Init_Response$Type();
var UploadAPI_UpdateProgress$Type = class extends MessageType {
	constructor() {
		super("MiLaboratories.Controller.Shared.UploadAPI.UpdateProgress", []);
	}
	create(value) {
		const message = globalThis.Object.create(this.messagePrototype);
		if (value !== void 0) reflectionMergePartial(this, message, value);
		return message;
	}
	internalBinaryRead(reader, length, options, target) {
		let message = target ?? this.create(), end = reader.pos + length;
		while (reader.pos < end) {
			let [fieldNo, wireType] = reader.tag();
			switch (fieldNo) {
				default:
					let u = options.readUnknownField;
					if (u === "throw") throw new globalThis.Error(`Unknown field ${fieldNo} (wire type ${wireType}) for ${this.typeName}`);
					let d = reader.skip(wireType);
					if (u !== false) (u === true ? UnknownFieldHandler.onRead : u)(this.typeName, message, fieldNo, wireType, d);
			}
		}
		return message;
	}
	internalBinaryWrite(message, writer, options) {
		let u = options.writeUnknownFields;
		if (u !== false) (u == true ? UnknownFieldHandler.onWrite : u)(this.typeName, message, writer);
		return writer;
	}
};
new UploadAPI_UpdateProgress$Type();
var UploadAPI_UpdateProgress_Request$Type = class extends MessageType {
	constructor() {
		super("MiLaboratories.Controller.Shared.UploadAPI.UpdateProgress.Request", [
			{
				no: 1,
				name: "resource_id",
				kind: "scalar",
				T: 4,
				L: 0
			},
			{
				no: 3,
				name: "resource_signature",
				kind: "scalar",
				opt: true,
				T: 12
			},
			{
				no: 2,
				name: "bytes_processed",
				kind: "scalar",
				T: 3,
				L: 0
			}
		]);
	}
	create(value) {
		const message = globalThis.Object.create(this.messagePrototype);
		message.resourceId = 0n;
		message.bytesProcessed = 0n;
		if (value !== void 0) reflectionMergePartial(this, message, value);
		return message;
	}
	internalBinaryRead(reader, length, options, target) {
		let message = target ?? this.create(), end = reader.pos + length;
		while (reader.pos < end) {
			let [fieldNo, wireType] = reader.tag();
			switch (fieldNo) {
				case 1:
					message.resourceId = reader.uint64().toBigInt();
					break;
				case 3:
					message.resourceSignature = reader.bytes();
					break;
				case 2:
					message.bytesProcessed = reader.int64().toBigInt();
					break;
				default:
					let u = options.readUnknownField;
					if (u === "throw") throw new globalThis.Error(`Unknown field ${fieldNo} (wire type ${wireType}) for ${this.typeName}`);
					let d = reader.skip(wireType);
					if (u !== false) (u === true ? UnknownFieldHandler.onRead : u)(this.typeName, message, fieldNo, wireType, d);
			}
		}
		return message;
	}
	internalBinaryWrite(message, writer, options) {
		if (message.resourceId !== 0n) writer.tag(1, WireType.Varint).uint64(message.resourceId);
		if (message.bytesProcessed !== 0n) writer.tag(2, WireType.Varint).int64(message.bytesProcessed);
		if (message.resourceSignature !== void 0) writer.tag(3, WireType.LengthDelimited).bytes(message.resourceSignature);
		let u = options.writeUnknownFields;
		if (u !== false) (u == true ? UnknownFieldHandler.onWrite : u)(this.typeName, message, writer);
		return writer;
	}
};
/**
* @generated MessageType for protobuf message MiLaboratories.Controller.Shared.UploadAPI.UpdateProgress.Request
*/
const UploadAPI_UpdateProgress_Request = new UploadAPI_UpdateProgress_Request$Type();
var UploadAPI_UpdateProgress_Response$Type = class extends MessageType {
	constructor() {
		super("MiLaboratories.Controller.Shared.UploadAPI.UpdateProgress.Response", []);
	}
	create(value) {
		const message = globalThis.Object.create(this.messagePrototype);
		if (value !== void 0) reflectionMergePartial(this, message, value);
		return message;
	}
	internalBinaryRead(reader, length, options, target) {
		let message = target ?? this.create(), end = reader.pos + length;
		while (reader.pos < end) {
			let [fieldNo, wireType] = reader.tag();
			switch (fieldNo) {
				default:
					let u = options.readUnknownField;
					if (u === "throw") throw new globalThis.Error(`Unknown field ${fieldNo} (wire type ${wireType}) for ${this.typeName}`);
					let d = reader.skip(wireType);
					if (u !== false) (u === true ? UnknownFieldHandler.onRead : u)(this.typeName, message, fieldNo, wireType, d);
			}
		}
		return message;
	}
	internalBinaryWrite(message, writer, options) {
		let u = options.writeUnknownFields;
		if (u !== false) (u == true ? UnknownFieldHandler.onWrite : u)(this.typeName, message, writer);
		return writer;
	}
};
/**
* @generated MessageType for protobuf message MiLaboratories.Controller.Shared.UploadAPI.UpdateProgress.Response
*/
const UploadAPI_UpdateProgress_Response = new UploadAPI_UpdateProgress_Response$Type();
var UploadAPI_GetPartURL$Type = class extends MessageType {
	constructor() {
		super("MiLaboratories.Controller.Shared.UploadAPI.GetPartURL", []);
	}
	create(value) {
		const message = globalThis.Object.create(this.messagePrototype);
		if (value !== void 0) reflectionMergePartial(this, message, value);
		return message;
	}
	internalBinaryRead(reader, length, options, target) {
		let message = target ?? this.create(), end = reader.pos + length;
		while (reader.pos < end) {
			let [fieldNo, wireType] = reader.tag();
			switch (fieldNo) {
				default:
					let u = options.readUnknownField;
					if (u === "throw") throw new globalThis.Error(`Unknown field ${fieldNo} (wire type ${wireType}) for ${this.typeName}`);
					let d = reader.skip(wireType);
					if (u !== false) (u === true ? UnknownFieldHandler.onRead : u)(this.typeName, message, fieldNo, wireType, d);
			}
		}
		return message;
	}
	internalBinaryWrite(message, writer, options) {
		let u = options.writeUnknownFields;
		if (u !== false) (u == true ? UnknownFieldHandler.onWrite : u)(this.typeName, message, writer);
		return writer;
	}
};
new UploadAPI_GetPartURL$Type();
var UploadAPI_GetPartURL_Request$Type = class extends MessageType {
	constructor() {
		super("MiLaboratories.Controller.Shared.UploadAPI.GetPartURL.Request", [
			{
				no: 1,
				name: "resource_id",
				kind: "scalar",
				T: 4,
				L: 0
			},
			{
				no: 6,
				name: "resource_signature",
				kind: "scalar",
				opt: true,
				T: 12
			},
			{
				no: 2,
				name: "part_number",
				kind: "scalar",
				T: 4,
				L: 0
			},
			{
				no: 3,
				name: "uploaded_part_size",
				kind: "scalar",
				T: 4,
				L: 0
			},
			{
				no: 4,
				name: "is_internal_use",
				kind: "scalar",
				T: 8
			},
			{
				no: 5,
				name: "part_checksum",
				kind: "scalar",
				T: 9
			}
		]);
	}
	create(value) {
		const message = globalThis.Object.create(this.messagePrototype);
		message.resourceId = 0n;
		message.partNumber = 0n;
		message.uploadedPartSize = 0n;
		message.isInternalUse = false;
		message.partChecksum = "";
		if (value !== void 0) reflectionMergePartial(this, message, value);
		return message;
	}
	internalBinaryRead(reader, length, options, target) {
		let message = target ?? this.create(), end = reader.pos + length;
		while (reader.pos < end) {
			let [fieldNo, wireType] = reader.tag();
			switch (fieldNo) {
				case 1:
					message.resourceId = reader.uint64().toBigInt();
					break;
				case 6:
					message.resourceSignature = reader.bytes();
					break;
				case 2:
					message.partNumber = reader.uint64().toBigInt();
					break;
				case 3:
					message.uploadedPartSize = reader.uint64().toBigInt();
					break;
				case 4:
					message.isInternalUse = reader.bool();
					break;
				case 5:
					message.partChecksum = reader.string();
					break;
				default:
					let u = options.readUnknownField;
					if (u === "throw") throw new globalThis.Error(`Unknown field ${fieldNo} (wire type ${wireType}) for ${this.typeName}`);
					let d = reader.skip(wireType);
					if (u !== false) (u === true ? UnknownFieldHandler.onRead : u)(this.typeName, message, fieldNo, wireType, d);
			}
		}
		return message;
	}
	internalBinaryWrite(message, writer, options) {
		if (message.resourceId !== 0n) writer.tag(1, WireType.Varint).uint64(message.resourceId);
		if (message.partNumber !== 0n) writer.tag(2, WireType.Varint).uint64(message.partNumber);
		if (message.uploadedPartSize !== 0n) writer.tag(3, WireType.Varint).uint64(message.uploadedPartSize);
		if (message.isInternalUse !== false) writer.tag(4, WireType.Varint).bool(message.isInternalUse);
		if (message.partChecksum !== "") writer.tag(5, WireType.LengthDelimited).string(message.partChecksum);
		if (message.resourceSignature !== void 0) writer.tag(6, WireType.LengthDelimited).bytes(message.resourceSignature);
		let u = options.writeUnknownFields;
		if (u !== false) (u == true ? UnknownFieldHandler.onWrite : u)(this.typeName, message, writer);
		return writer;
	}
};
/**
* @generated MessageType for protobuf message MiLaboratories.Controller.Shared.UploadAPI.GetPartURL.Request
*/
const UploadAPI_GetPartURL_Request = new UploadAPI_GetPartURL_Request$Type();
var UploadAPI_GetPartURL_HTTPHeader$Type = class extends MessageType {
	constructor() {
		super("MiLaboratories.Controller.Shared.UploadAPI.GetPartURL.HTTPHeader", [{
			no: 1,
			name: "name",
			kind: "scalar",
			T: 9
		}, {
			no: 2,
			name: "value",
			kind: "scalar",
			T: 9
		}]);
	}
	create(value) {
		const message = globalThis.Object.create(this.messagePrototype);
		message.name = "";
		message.value = "";
		if (value !== void 0) reflectionMergePartial(this, message, value);
		return message;
	}
	internalBinaryRead(reader, length, options, target) {
		let message = target ?? this.create(), end = reader.pos + length;
		while (reader.pos < end) {
			let [fieldNo, wireType] = reader.tag();
			switch (fieldNo) {
				case 1:
					message.name = reader.string();
					break;
				case 2:
					message.value = reader.string();
					break;
				default:
					let u = options.readUnknownField;
					if (u === "throw") throw new globalThis.Error(`Unknown field ${fieldNo} (wire type ${wireType}) for ${this.typeName}`);
					let d = reader.skip(wireType);
					if (u !== false) (u === true ? UnknownFieldHandler.onRead : u)(this.typeName, message, fieldNo, wireType, d);
			}
		}
		return message;
	}
	internalBinaryWrite(message, writer, options) {
		if (message.name !== "") writer.tag(1, WireType.LengthDelimited).string(message.name);
		if (message.value !== "") writer.tag(2, WireType.LengthDelimited).string(message.value);
		let u = options.writeUnknownFields;
		if (u !== false) (u == true ? UnknownFieldHandler.onWrite : u)(this.typeName, message, writer);
		return writer;
	}
};
/**
* @generated MessageType for protobuf message MiLaboratories.Controller.Shared.UploadAPI.GetPartURL.HTTPHeader
*/
const UploadAPI_GetPartURL_HTTPHeader = new UploadAPI_GetPartURL_HTTPHeader$Type();
var UploadAPI_GetPartURL_Response$Type = class extends MessageType {
	constructor() {
		super("MiLaboratories.Controller.Shared.UploadAPI.GetPartURL.Response", [
			{
				no: 1,
				name: "upload_url",
				kind: "scalar",
				T: 9
			},
			{
				no: 2,
				name: "method",
				kind: "scalar",
				T: 9
			},
			{
				no: 3,
				name: "headers",
				kind: "message",
				repeat: 2,
				T: () => UploadAPI_GetPartURL_HTTPHeader
			},
			{
				no: 4,
				name: "chunk_start",
				kind: "scalar",
				T: 4,
				L: 0
			},
			{
				no: 5,
				name: "chunk_end",
				kind: "scalar",
				T: 4,
				L: 0
			}
		]);
	}
	create(value) {
		const message = globalThis.Object.create(this.messagePrototype);
		message.uploadUrl = "";
		message.method = "";
		message.headers = [];
		message.chunkStart = 0n;
		message.chunkEnd = 0n;
		if (value !== void 0) reflectionMergePartial(this, message, value);
		return message;
	}
	internalBinaryRead(reader, length, options, target) {
		let message = target ?? this.create(), end = reader.pos + length;
		while (reader.pos < end) {
			let [fieldNo, wireType] = reader.tag();
			switch (fieldNo) {
				case 1:
					message.uploadUrl = reader.string();
					break;
				case 2:
					message.method = reader.string();
					break;
				case 3:
					message.headers.push(UploadAPI_GetPartURL_HTTPHeader.internalBinaryRead(reader, reader.uint32(), options));
					break;
				case 4:
					message.chunkStart = reader.uint64().toBigInt();
					break;
				case 5:
					message.chunkEnd = reader.uint64().toBigInt();
					break;
				default:
					let u = options.readUnknownField;
					if (u === "throw") throw new globalThis.Error(`Unknown field ${fieldNo} (wire type ${wireType}) for ${this.typeName}`);
					let d = reader.skip(wireType);
					if (u !== false) (u === true ? UnknownFieldHandler.onRead : u)(this.typeName, message, fieldNo, wireType, d);
			}
		}
		return message;
	}
	internalBinaryWrite(message, writer, options) {
		if (message.uploadUrl !== "") writer.tag(1, WireType.LengthDelimited).string(message.uploadUrl);
		if (message.method !== "") writer.tag(2, WireType.LengthDelimited).string(message.method);
		for (let i = 0; i < message.headers.length; i++) UploadAPI_GetPartURL_HTTPHeader.internalBinaryWrite(message.headers[i], writer.tag(3, WireType.LengthDelimited).fork(), options).join();
		if (message.chunkStart !== 0n) writer.tag(4, WireType.Varint).uint64(message.chunkStart);
		if (message.chunkEnd !== 0n) writer.tag(5, WireType.Varint).uint64(message.chunkEnd);
		let u = options.writeUnknownFields;
		if (u !== false) (u == true ? UnknownFieldHandler.onWrite : u)(this.typeName, message, writer);
		return writer;
	}
};
/**
* @generated MessageType for protobuf message MiLaboratories.Controller.Shared.UploadAPI.GetPartURL.Response
*/
const UploadAPI_GetPartURL_Response = new UploadAPI_GetPartURL_Response$Type();
var UploadAPI_Finalize$Type = class extends MessageType {
	constructor() {
		super("MiLaboratories.Controller.Shared.UploadAPI.Finalize", []);
	}
	create(value) {
		const message = globalThis.Object.create(this.messagePrototype);
		if (value !== void 0) reflectionMergePartial(this, message, value);
		return message;
	}
	internalBinaryRead(reader, length, options, target) {
		let message = target ?? this.create(), end = reader.pos + length;
		while (reader.pos < end) {
			let [fieldNo, wireType] = reader.tag();
			switch (fieldNo) {
				default:
					let u = options.readUnknownField;
					if (u === "throw") throw new globalThis.Error(`Unknown field ${fieldNo} (wire type ${wireType}) for ${this.typeName}`);
					let d = reader.skip(wireType);
					if (u !== false) (u === true ? UnknownFieldHandler.onRead : u)(this.typeName, message, fieldNo, wireType, d);
			}
		}
		return message;
	}
	internalBinaryWrite(message, writer, options) {
		let u = options.writeUnknownFields;
		if (u !== false) (u == true ? UnknownFieldHandler.onWrite : u)(this.typeName, message, writer);
		return writer;
	}
};
new UploadAPI_Finalize$Type();
var UploadAPI_Finalize_Request$Type = class extends MessageType {
	constructor() {
		super("MiLaboratories.Controller.Shared.UploadAPI.Finalize.Request", [
			{
				no: 1,
				name: "resource_id",
				kind: "scalar",
				T: 4,
				L: 0
			},
			{
				no: 4,
				name: "resource_signature",
				kind: "scalar",
				opt: true,
				T: 12
			},
			{
				no: 2,
				name: "checksum_algorithm",
				kind: "enum",
				T: () => [
					"MiLaboratories.Controller.Shared.UploadAPI.ChecksumAlgorithm",
					UploadAPI_ChecksumAlgorithm,
					"CHECKSUM_ALGORITHM_"
				]
			},
			{
				no: 3,
				name: "checksum",
				kind: "scalar",
				T: 12
			}
		]);
	}
	create(value) {
		const message = globalThis.Object.create(this.messagePrototype);
		message.resourceId = 0n;
		message.checksumAlgorithm = 0;
		message.checksum = new Uint8Array(0);
		if (value !== void 0) reflectionMergePartial(this, message, value);
		return message;
	}
	internalBinaryRead(reader, length, options, target) {
		let message = target ?? this.create(), end = reader.pos + length;
		while (reader.pos < end) {
			let [fieldNo, wireType] = reader.tag();
			switch (fieldNo) {
				case 1:
					message.resourceId = reader.uint64().toBigInt();
					break;
				case 4:
					message.resourceSignature = reader.bytes();
					break;
				case 2:
					message.checksumAlgorithm = reader.int32();
					break;
				case 3:
					message.checksum = reader.bytes();
					break;
				default:
					let u = options.readUnknownField;
					if (u === "throw") throw new globalThis.Error(`Unknown field ${fieldNo} (wire type ${wireType}) for ${this.typeName}`);
					let d = reader.skip(wireType);
					if (u !== false) (u === true ? UnknownFieldHandler.onRead : u)(this.typeName, message, fieldNo, wireType, d);
			}
		}
		return message;
	}
	internalBinaryWrite(message, writer, options) {
		if (message.resourceId !== 0n) writer.tag(1, WireType.Varint).uint64(message.resourceId);
		if (message.checksumAlgorithm !== 0) writer.tag(2, WireType.Varint).int32(message.checksumAlgorithm);
		if (message.checksum.length) writer.tag(3, WireType.LengthDelimited).bytes(message.checksum);
		if (message.resourceSignature !== void 0) writer.tag(4, WireType.LengthDelimited).bytes(message.resourceSignature);
		let u = options.writeUnknownFields;
		if (u !== false) (u == true ? UnknownFieldHandler.onWrite : u)(this.typeName, message, writer);
		return writer;
	}
};
/**
* @generated MessageType for protobuf message MiLaboratories.Controller.Shared.UploadAPI.Finalize.Request
*/
const UploadAPI_Finalize_Request = new UploadAPI_Finalize_Request$Type();
var UploadAPI_Finalize_Response$Type = class extends MessageType {
	constructor() {
		super("MiLaboratories.Controller.Shared.UploadAPI.Finalize.Response", []);
	}
	create(value) {
		const message = globalThis.Object.create(this.messagePrototype);
		if (value !== void 0) reflectionMergePartial(this, message, value);
		return message;
	}
	internalBinaryRead(reader, length, options, target) {
		let message = target ?? this.create(), end = reader.pos + length;
		while (reader.pos < end) {
			let [fieldNo, wireType] = reader.tag();
			switch (fieldNo) {
				default:
					let u = options.readUnknownField;
					if (u === "throw") throw new globalThis.Error(`Unknown field ${fieldNo} (wire type ${wireType}) for ${this.typeName}`);
					let d = reader.skip(wireType);
					if (u !== false) (u === true ? UnknownFieldHandler.onRead : u)(this.typeName, message, fieldNo, wireType, d);
			}
		}
		return message;
	}
	internalBinaryWrite(message, writer, options) {
		let u = options.writeUnknownFields;
		if (u !== false) (u == true ? UnknownFieldHandler.onWrite : u)(this.typeName, message, writer);
		return writer;
	}
};
/**
* @generated MessageType for protobuf message MiLaboratories.Controller.Shared.UploadAPI.Finalize.Response
*/
const UploadAPI_Finalize_Response = new UploadAPI_Finalize_Response$Type();
var UploadAPI_Reset$Type = class extends MessageType {
	constructor() {
		super("MiLaboratories.Controller.Shared.UploadAPI.Reset", []);
	}
	create(value) {
		const message = globalThis.Object.create(this.messagePrototype);
		if (value !== void 0) reflectionMergePartial(this, message, value);
		return message;
	}
	internalBinaryRead(reader, length, options, target) {
		let message = target ?? this.create(), end = reader.pos + length;
		while (reader.pos < end) {
			let [fieldNo, wireType] = reader.tag();
			switch (fieldNo) {
				default:
					let u = options.readUnknownField;
					if (u === "throw") throw new globalThis.Error(`Unknown field ${fieldNo} (wire type ${wireType}) for ${this.typeName}`);
					let d = reader.skip(wireType);
					if (u !== false) (u === true ? UnknownFieldHandler.onRead : u)(this.typeName, message, fieldNo, wireType, d);
			}
		}
		return message;
	}
	internalBinaryWrite(message, writer, options) {
		let u = options.writeUnknownFields;
		if (u !== false) (u == true ? UnknownFieldHandler.onWrite : u)(this.typeName, message, writer);
		return writer;
	}
};
new UploadAPI_Reset$Type();
var UploadAPI_Reset_Request$Type = class extends MessageType {
	constructor() {
		super("MiLaboratories.Controller.Shared.UploadAPI.Reset.Request", [{
			no: 1,
			name: "resource_id",
			kind: "scalar",
			T: 4,
			L: 0
		}, {
			no: 2,
			name: "resource_signature",
			kind: "scalar",
			opt: true,
			T: 12
		}]);
	}
	create(value) {
		const message = globalThis.Object.create(this.messagePrototype);
		message.resourceId = 0n;
		if (value !== void 0) reflectionMergePartial(this, message, value);
		return message;
	}
	internalBinaryRead(reader, length, options, target) {
		let message = target ?? this.create(), end = reader.pos + length;
		while (reader.pos < end) {
			let [fieldNo, wireType] = reader.tag();
			switch (fieldNo) {
				case 1:
					message.resourceId = reader.uint64().toBigInt();
					break;
				case 2:
					message.resourceSignature = reader.bytes();
					break;
				default:
					let u = options.readUnknownField;
					if (u === "throw") throw new globalThis.Error(`Unknown field ${fieldNo} (wire type ${wireType}) for ${this.typeName}`);
					let d = reader.skip(wireType);
					if (u !== false) (u === true ? UnknownFieldHandler.onRead : u)(this.typeName, message, fieldNo, wireType, d);
			}
		}
		return message;
	}
	internalBinaryWrite(message, writer, options) {
		if (message.resourceId !== 0n) writer.tag(1, WireType.Varint).uint64(message.resourceId);
		if (message.resourceSignature !== void 0) writer.tag(2, WireType.LengthDelimited).bytes(message.resourceSignature);
		let u = options.writeUnknownFields;
		if (u !== false) (u == true ? UnknownFieldHandler.onWrite : u)(this.typeName, message, writer);
		return writer;
	}
};
/**
* @generated MessageType for protobuf message MiLaboratories.Controller.Shared.UploadAPI.Reset.Request
*/
const UploadAPI_Reset_Request = new UploadAPI_Reset_Request$Type();
var UploadAPI_Reset_Response$Type = class extends MessageType {
	constructor() {
		super("MiLaboratories.Controller.Shared.UploadAPI.Reset.Response", []);
	}
	create(value) {
		const message = globalThis.Object.create(this.messagePrototype);
		if (value !== void 0) reflectionMergePartial(this, message, value);
		return message;
	}
	internalBinaryRead(reader, length, options, target) {
		let message = target ?? this.create(), end = reader.pos + length;
		while (reader.pos < end) {
			let [fieldNo, wireType] = reader.tag();
			switch (fieldNo) {
				default:
					let u = options.readUnknownField;
					if (u === "throw") throw new globalThis.Error(`Unknown field ${fieldNo} (wire type ${wireType}) for ${this.typeName}`);
					let d = reader.skip(wireType);
					if (u !== false) (u === true ? UnknownFieldHandler.onRead : u)(this.typeName, message, fieldNo, wireType, d);
			}
		}
		return message;
	}
	internalBinaryWrite(message, writer, options) {
		let u = options.writeUnknownFields;
		if (u !== false) (u == true ? UnknownFieldHandler.onWrite : u)(this.typeName, message, writer);
		return writer;
	}
};
/**
* @generated MessageType for protobuf message MiLaboratories.Controller.Shared.UploadAPI.Reset.Response
*/
const UploadAPI_Reset_Response = new UploadAPI_Reset_Response$Type();
/**
* @generated ServiceType for protobuf service MiLaboratories.Controller.Shared.Upload
*/
const Upload = new ServiceType("MiLaboratories.Controller.Shared.Upload", [
	{
		name: "Init",
		options: { "google.api.http": {
			post: "/v1/upload/init",
			body: "*"
		} },
		I: UploadAPI_Init_Request,
		O: UploadAPI_Init_Response
	},
	{
		name: "GetPartURL",
		options: { "google.api.http": {
			post: "/v1/upload/get-part-url",
			body: "*"
		} },
		I: UploadAPI_GetPartURL_Request,
		O: UploadAPI_GetPartURL_Response
	},
	{
		name: "UpdateProgress",
		options: { "google.api.http": {
			post: "/v1/upload/update-progress",
			body: "*"
		} },
		I: UploadAPI_UpdateProgress_Request,
		O: UploadAPI_UpdateProgress_Response
	},
	{
		name: "Finalize",
		options: { "google.api.http": {
			post: "/v1/upload/finalize",
			body: "*"
		} },
		I: UploadAPI_Finalize_Request,
		O: UploadAPI_Finalize_Response
	},
	{
		name: "Reset",
		options: { "google.api.http": {
			post: "/v1/upload/reset",
			body: "*"
		} },
		I: UploadAPI_Reset_Request,
		O: UploadAPI_Reset_Response
	}
]);
//#endregion
export { Upload, UploadAPI_ChecksumAlgorithm };

//# sourceMappingURL=protocol.js.map