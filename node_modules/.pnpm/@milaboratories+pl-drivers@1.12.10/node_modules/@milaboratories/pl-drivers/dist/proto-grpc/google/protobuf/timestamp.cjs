...
var Timestamp$Type = class extends _protobuf_ts_runtime.MessageType {
	...
	*/
	now() {
		...
		const ms = Date.now();
		msg.seconds = _protobuf_ts_runtime.PbLong.from(Math.floor(ms / 1e3)).toBigInt();
	...
	*/
	toDate(message) {
		return new Date(_protobuf_ts_runtime.PbLong.from(message.seconds).toNumber() * 1e3 + Math.ceil(message.nanos / 1e6));
	}
	/**
	...
	*/
	internalJsonWrite(message, options) {
		let ms = _protobuf_ts_runtime.PbLong.from(message.seconds).toNumber() * 1e3;
	...
	*/
	internalJsonRead(json, options, target) {
		if (typeof json !== "string") throw new Error("Unable to parse Timestamp from JSON " + (0, _protobuf_ts_runtime.typeofJsonValue)(json) + ".");
		...
		target.seconds = _protobuf_ts_runtime.PbLong.from(ms / 1e3).toBigInt();
		target.nanos = 0;
	...
	internalBinaryWrite(message, writer, options) {
		if (message.seconds !== 0n) writer.tag(1, _protobuf_ts_runtime.WireType.Varint).int64(message.seconds);
		if (message.nanos !== 0) writer.tag(2, _protobuf_ts_runtime.WireType.Varint).int32(message.nanos);
