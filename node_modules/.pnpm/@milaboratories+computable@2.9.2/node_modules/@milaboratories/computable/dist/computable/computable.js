import { createCellState, createCellStateWithoutValue, destroyState, updateCellState, updateCellStateWithoutValue } from "./computable_state.js";
...
var Computable = class Computable {
	...
	* */
	async awaitChange(abortSignal, uTag) {
		if (this._changed(uTag)) {
			const hooks = this.state?.hooks;
			if (hooks !== void 0) for (const h of hooks) h.onChangedRequest(this);
			await setImmediate(void 0, { signal: abortSignal });
			return;
		}
	...
	*/
	async awaitStableFullValue(abortSignalOrTimeout) {
		try {
			...
			if (e.name === "AbortError") throw new Aborted({
				cause: e,
	...
	* */
	_preCalculateValueTree() {
		...
		this.state = this.state === void 0 ? createCellStateWithoutValue(this.___wrapped_kernel___) : updateCellStateWithoutValue(this.state);
	...
	async getValueOrError() {
		...
		if (this.stateCalculation !== void 0) await this.stateCalculation;
			...
			this.stateCalculation = (async () => {
				try {
					const newState = this.state === void 0 ? await createCellState(this.___wrapped_kernel___) : await updateCellState(this.state);
	...
	*/
	resetState() {
		...
		else if (this.state !== void 0) destroyState(this.state);
		this.state = void 0;
		this.epoch++;
		this.uTag = "";
	}
...
//#endregion
export { AggregateComputableError, Computable, ComputableError };
