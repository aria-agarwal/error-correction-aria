	...
	constructor(logger, ops) {
		...
		this.global = new lru_cache.LRUCache({
			...
			dispose: (resource, key, reason) => {
				resource.unref();
				if (require_logging.logPFrames()) logger("info", `createPTable cache - removed PTable ${key} (reason: ${reason})`);
			}
		});
	}
	cache(resource, size, defDisposeSignal) {
		const key = resource.key;
		if (require_logging.logPFrames()) this.logger("info", `createPTable cache - added PTable ${key} with size ${size}`);
		const status = {};
		...
		if (status.maxEntrySizeExceeded) {
			resource.unref();
			if (require_logging.logPFrames()) this.logger("info", `createPTable cache - removed PTable ${key} (maxEntrySizeExceeded)`);
