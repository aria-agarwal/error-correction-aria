...
*/
function findNamedErrorInCauses(err, errorConstructor, maxDepth = 10) {
	return findNamedErrorInternal(err, errorConstructor, maxDepth);
}
function findNamedErrorInternal(err, errorConstructor, maxDepth = 10, depth = 0) {
	if (err instanceof errorConstructor) return err;
	if (err?.name === errorConstructor.name) return err;
	if (depth >= maxDepth) return void 0;
	if (err?.cause) return findNamedErrorInternal(err.cause, errorConstructor, maxDepth, depth + 1);
}

//#endregion
exports.findNamedErrorInCauses = findNamedErrorInCauses;
