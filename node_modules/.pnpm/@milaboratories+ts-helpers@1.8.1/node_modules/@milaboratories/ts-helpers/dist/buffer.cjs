...
const require_obj = require('./obj.cjs');
...
const decodingCache = /* @__PURE__ */ new WeakMap();
...
function getCachedDeserializeStats() {
	return {
...
function cachedDecode(data) {
	...
	const dataToDecode = data.length >= 2 && data[0] === 31 && data[1] === 139 ? (0, node_zlib.gunzipSync)(data) : data;
	const text = textDecoder.decode(dataToDecode);
	if (data.byteLength >= CachedMinCachingSize) decodingCache.set(data, text);
	numberOfDecodings++;
...
*/
function cachedDeserialize(data) {
...
*/
function canonicalJsonBytes(data) {
	const canonicalJson = (0, canonicalize.default)(data);
	if (canonicalJson === void 0) throw new Error(`The data cannot be converted to canonical JSON. ${data}`);
...
* @throws If the data cannot be converted to canonical JSON.
*/
function canonicalJsonGzBytes(data, minSizeToGzip = 16384) {
	const jsonData = canonicalJsonBytes(data);
	if (minSizeToGzip === -1 || jsonData.length >= minSizeToGzip) return {
		data: (0, node_zlib.gzipSync)(jsonData),
		isGzipped: true
	};
	return {
		data: jsonData,
...
//#endregion
exports.cachedDecode = cachedDecode;
exports.cachedDeserialize = cachedDeserialize;
exports.canonicalJsonBytes = canonicalJsonBytes;
exports.canonicalJsonGzBytes = canonicalJsonGzBytes;
exports.getCachedDecodeStats = getCachedDecodeStats;
exports.getCachedDeserializeStats = getCachedDeserializeStats;
