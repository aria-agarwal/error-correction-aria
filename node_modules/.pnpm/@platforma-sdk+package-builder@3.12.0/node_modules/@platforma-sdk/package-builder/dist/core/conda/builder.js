import { assertNever, currentOS, currentPlatform, splitPlatform } from "../util.js";
...
var micromamba = class {
	constructor(logger, rootPrefix, binaryVersion = "latest", binaryPath = "") {
		...
		if (!binaryPath) this.binaryPath = path.join(rootPrefix, "micromamba");
		if (currentOS() === "windows" && !this.binaryPath.endsWith(".exe")) this.binaryPath += ".exe";
	}
	async downloadBinary() {
		if (fs.existsSync(this.binaryPath)) try {
			...
			await fsp.chmod(this.binaryPath, 493);
		}
	...
	exportEnvironment(opts) {
		...
		try {
			...
			if (file !== "pipe") fs.closeSync(file);
	...
	deleteEnvironment(opts) {
		...
		if (envInfo.prefix) {
			this.logger.debug(`Deleting conda environment '${environmentName ?? environmentPrefix ?? defaultEnvironmentName}'...`);
			fs.rmSync(envInfo.prefix, { recursive: true });
		} else {
...
async function downloadMicromamba(logger, options) {
	...
	await fsp.chmod(binaryPath, 493);
...
//#endregion
export { micromamba, micromambaDownloadUrl };
