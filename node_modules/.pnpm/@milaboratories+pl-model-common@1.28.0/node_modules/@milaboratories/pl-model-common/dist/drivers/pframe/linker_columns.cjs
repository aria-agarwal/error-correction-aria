...
//#region src/drivers/pframe/linker_columns.ts
var LinkerMap = class LinkerMap {
	...
	get keyAxesIds() {
		return [...this.data.keys()].map(require_json.parseJson);
	}
	static fromColumns(columns) {
		const result = /* @__PURE__ */ new Map();
		for (const linker of columns.filter((l) => !!require_spec.readAnnotationJson(l.spec, require_spec.Annotation.IsLinkerColumn))) {
			const groups = LinkerMap.getAxesGroups(require_spec.getNormalizedAxesList(linker.spec.axesSpec));
			if (groups.length !== 2) continue;
			const [left, right] = groups;
			const leftKeyVariants = LinkerMap.getAxesRoots(left).map((axis) => {
				const axes = require_spec.getArrayFromAxisTree(require_spec.getAxesTree(axis));
				return [require_json.canonicalizeJson(axes.map(require_spec.getAxisId)), axes];
			});
	...
	getAxesListFromKeysList(keys) {
		return Array.from(new Map(keys.flatMap((key) => this.data.get(key)?.keyAxesSpec ?? []).map((axis) => [require_json.canonicalizeJson(require_spec.getAxisId(axis)), axis])).values());
	}
	...
	getNonLinkableAxes(sourceAxes, targetAxes) {
		...
		}).flatMap((axis) => require_spec.getArrayFromAxisTree(require_spec.getAxesTree(axis)).map((axis) => [require_json.canonicalizeJson(require_spec.getAxisId(axis)), axis]))).values());
	}
	...
	getReachableByLinkersAxesFromAxesNormalized(sourceAxes, matchAxisIdFn) {
		let startKeys = [];
		if (matchAxisIdFn) {
			const sourceAxisIdsGrouped = sourceAxes.map((axis) => require_spec.getArrayFromAxisTree(require_spec.getAxesTree(axis)).map(require_spec.getAxisId));
			for (const sourceAxisIdsGroup of sourceAxisIdsGrouped) {
				...
				if (matched) startKeys.push(require_json.canonicalizeJson(matched));
			}
	...
	getReachableByLinkersAxesFromAxes(sourceAxes, matchAxisIdFn) {
		return this.getReachableByLinkersAxesFromAxesNormalized(require_spec.getNormalizedAxesList(sourceAxes), matchAxisIdFn);
	}
	static getLinkerKeyFromAxisSpec(axis) {
		return require_json.canonicalizeJson(require_spec.getArrayFromAxisTree(require_spec.getAxesTree(axis)).map(require_spec.getAxisId));
	}
	...
	static getAxesGroups(axesSpec) {
		...
		const axisKeys = axesSpec.map((spec) => require_json.canonicalizeJson(require_spec.getAxisId(spec)));
		const axisParentsIdxs = axesSpec.map((spec) => new Set(spec.parentAxesSpec.map((spec) => require_json.canonicalizeJson(require_spec.getAxisId(spec))).map((el) => {
	...
	static getAxesRoots(axes) {
		const parentsSet = new Set(axes.flatMap((axis) => axis.parentAxesSpec).map((spec) => require_json.canonicalizeJson(require_spec.getAxisId(spec))));
		return axes.filter((axis) => !parentsSet.has(require_json.canonicalizeJson(require_spec.getAxisId(axis))));
	}
};

//#endregion
exports.LinkerMap = LinkerMap;
//# sourceMappingURL=linker_columns.cjs.map